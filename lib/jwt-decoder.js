...
class JwtDecoder {
    ...
    static bearerTokenExpired(token) {
        logger_1.Logger.log.trace("JwtDecoder.bearerTokenExpired: start");
        logger_1.Logger.log.trace("JwtDecoder.bearerTokenExpired: Checking bearer token for expiry.");
        ...
        if (!decoder.isValid) {
            ...
            logger_1.Logger.log.trace(`JwtDecoder.bearerTokenExpired: Bearer token expiry: ${decoder.isExpired}`);
     ...
     */
    decodeToken() {
        logger_1.Logger.log.trace("JwtDecoder.decodeToken: start");
        // check for the jwt
        logger_1.Logger.log.trace("JwtDecoder.decodeToken: Checking that there is a JWT.");
        ...
        logger_1.Logger.log.trace("JwtDecoder.decodeToken: Checking that the JWT has 3 parts.");
        if (parts.length !== 3) {
        ...
        logger_1.Logger.log.trace("JwtDecoder.decodeToken: Decoding the claims.");
        let decodedClaims = new Buffer(parts[1], "base64").toString();
        logger_1.Logger.log.trace("JwtDecoder.decodeToken: The JWT claims: ", { data: decodedClaims });
    ...
    readTokenExpirationDate() {
        logger_1.Logger.log.trace("JwtDecoder.readTokenExpirationDate: start");
        ...
        try {
            ...
            logger_1.Logger.log.trace(`JwtDecoder.readTokenExpirationDate: Bearer token expiry: ${date}`);
        }
        catch (e) {
    ...
    isTokenExpired(offsetSeconds) {
        logger_1.Logger.log.trace("JwtDecoder.isTokenExpired: start");
