...
class AesEncryptor {
    ...
    deriveKeyAndIV(data, format) {
        ...
        let result = new EncryptionParameters();
        // convert the data into bytes.
        result.data = new Buffer(data, format);
        logger_1.Logger.log.debug(`AesIVDecryptor.deriveKeyAndIV: Data (base64): ${result.data.toString("base64")}`);
        // the salt is bytes 8-15.
        result.salt = result.data.slice(8, 16);
        logger_1.Logger.log.debug(`AesIVDecryptor.deriveKeyAndIV: Salt (hex): ${result.salt.toString("hex")}`);
        ...
        let rounds = 3;
        let data00 = Buffer.concat([new Buffer(this.passphrase, "utf8"), result.salt]);
        ...
        result.iv = md5hash[2];
        logger_1.Logger.log.debug(`AesIVDecryptor.deriveKeyAndIV: Key (hex): ${result.key.toString("hex")}`);
        logger_1.Logger.log.debug(`AesIVDecryptor.deriveKeyAndIV: IV (hex): ${result.iv.toString("hex")}`);
        return result;
    }
    /**
    ...
    createKeyAndIV() {
        ...
        logger_1.Logger.log.debug(`AesIVDecryptor.createKeyAndIV: Salt (hex): ${result.salt.toString("hex")}`);
        // the key and the IV are derived through 3 rounds of MD5 hashing, each generating 8 bytes of data.
        let rounds = 3;
        let data00 = Buffer.concat([new Buffer(this.passphrase, "utf8"), result.salt]);
        let md5hash = [new Buffer(Crypto.createHash("md5").update(data00).digest("base64"), "base64")];
        ...
        result.key = Buffer.concat([md5hash[0], md5hash[1]]);
        result.iv = md5hash[2];
        logger_1.Logger.log.debug(`AesIVDecryptor.createKeyAndIV: Key (hex): ${result.key.toString("hex")}`);
        logger_1.Logger.log.debug(`AesIVDecryptor.createKeyAndIV: IV (hex): ${result.iv.toString("hex")}`);
        return result;
    }
}
