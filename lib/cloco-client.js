...
class ClocoClient {
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.Logger.log.trace("ClocoClient.init: start.");
            try {
                // load application.
                ...
                logger_1.Logger.log.trace("ClocoClient.init: initialization complete.");
            }
            catch (e) {
     ...
     */
    get(objectId) {
        logger_1.Logger.log.trace(`ClocoClient.get: retrieving object '${objectId}'.`);
        if (cache_1.Cache.current.exists(objectId)) {
            logger_1.Logger.log.trace(`ClocoClient.get: object '${objectId}' found in cache.  Returning.`);
            return cache_1.Cache.current.get(objectId);
        }
        else {
            logger_1.Logger.log.trace(`ClocoClient.get: object '${objectId}' not found in cache.`);
            return undefined;
        }
    }
    /**
    ...
    put(objectId, item) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.Logger.log.trace(`ClocoClient.put: writing object '${objectId}'.`);
            // first, write to the API.
            logger_1.Logger.log.trace(`ClocoClient.put: writing object '${objectId}' to cloco API.`);
            ...
            logger_1.Logger.log.trace(`ClocoClient.put: finished api call for '${objectId}'.`, { data: wrapper }, { format: typeof wrapper });
            // then put the item in cache.
            logger_1.Logger.log.trace(`ClocoClient.put: adding object '${objectId}' to cache.`);
    ...
    loadApplication() {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.Logger.log.trace(`ClocoClient.loadApplication: start.`);
            try {
            ...
            logger_1.Logger.log.trace(`ClocoClient.loadApplication: complete.`);
        });
    }
    /**
    ...
    loadConfigurationFromApi() {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.Logger.log.trace(`ClocoClient.loadConfigurationFromApi: start.`);
            for (let i = 0; i < this.app.configObjects.length; i++) {
                logger_1.Logger.log.trace(`ClocoClient.loadConfigurationFromApi: Requesting configuration item '${this.app.configObjects[i].objectIdentifier}'.`);
                yield this.loadConfigurationObjectWrapperFromApi(this.app.configObjects[i].objectIdentifier);
            }
            logger_1.Logger.log.trace(`ClocoClient.loadConfigurationFromApi: complete.`);
        });
    }
    /**
    ...
    loadConfigurationObjectWrapperFromApi(objectId) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.Logger.log.trace(`ClocoClient.loadConfigurationObjectWrapperFromApi: start.`);
            try {
                logger_1.Logger.log.trace(`ClocoClient.loadConfigurationObjectWrapperFromApi: Checking bearer token.`);
                yield this.checkBearerToken();
                logger_1.Logger.log.trace(`ClocoClient.loadConfigurationObjectWrapperFromApi: Requesting configuration item '${objectId}'.`);
                let wrapper = yield api_client_1.ApiClient.getConfigObject(this.options, objectId);
                logger_1.Logger.log.trace(`ClocoClient.loadConfigurationObjectWrapperFromApi: Received configuration item '${objectId}'.`, wrapper);
                logger_1.Logger.log.trace(
            ...
            logger_1.Logger.log.trace(`ClocoClient.loadConfigurationObjectWrapperFromApi: complete.`);
        });
    }
    /**
     ...
     */
    checkCacheTimeouts() {
        logger_1.Logger.log.trace(`ClocoClient.checkCacheTimeouts: start.`);
        for (let i = 0; i < cache_1.Cache.current.items.length; i++) {
            if (cache_1.Cache.current.items[i].isExpired()) {
                logger_1.Logger.log.trace(`ClocoClient.checkCacheTimeouts: cache expired for item '${cache_1.Cache.current.items[i].key}'.`, { data: cache_1.Cache.current.items[i] });
                this.onCacheExpired.dispatch(this, cache_1.Cache.current.items[i]);
            }
        }
        logger_1.Logger.log.trace(`ClocoClient.checkCacheTimeouts: complete.`);
        this.timer = setTimeout(() => this.checkCacheTimeouts(), this.options.cacheCheckInterval);
    }
    /**
    ...
    checkBearerToken() {
        return __awaiter(this, void 0, void 0, function* () {
            logger_1.Logger.log.trace(`ClocoClient.checkBearerToken: start.`);
            try {
                if (jwt_decoder_1.JwtDecoder.bearerTokenExpired(this.options.tokens.accessToken)) {
                    logger_1.Logger.log.trace(`ClocoClient.checkBearerToken: bearer token has expired and will be refreshed.`);
                    let response;
            ...
            logger_1.Logger.log.trace(`ClocoClient.checkBearerToken: complete.`);
        });
    }
    /**
    ...
    environmentExists(environmentId) {
        logger_1.Logger.log.trace(`ClocoClient.environmentExists: start. Searching for '${environmentId}'.`);
        if (!this.app || !this.app.environments || !environmentId) {
            logger_1.Logger.log.trace("ClocoClient.environmentExists: data not available, returning false.");
            return false;
        }
        for (let i = 0; i < this.app.environments.length; i++) {
            logger_1.Logger.log.trace(this.app.environments[i], "ClocoClient.environmentExists: object found.");
            if (this.app.environments[i].environmentIdentifier === environmentId) {
                return true;
            }
        }
        logger_1.Logger.log.trace("ClocoClient.environmentExists: environment not found, returning false.");
        return false;
    }
    /**
    ...
    getConfigObject(objectId) {
        logger_1.Logger.log.trace(`ClocoClient.getConfigObject: start. Searching for '${objectId}'.`);
        if (!this.app || !this.app.configObjects) {
            logger_1.Logger.log.trace("ClocoClient.getConfigObject: app not available, returning undefined.");
            return undefined;
        }
        else {
            for (let i = 0; i < this.app.configObjects.length; i++) {
                if (this.app.configObjects[i].objectIdentifier === objectId) {
                    logger_1.Logger.log.trace(this.app.configObjects[i], "ClocoClient.getConfigObject: object found.");
                    return this.app.configObjects[i];
                }
            }
        }
        logger_1.Logger.log.trace("ClocoClient.getConfigObject: object not found, returning undefined.");
        return undefined;
    }
    /**
    ...
    encodeAndEncrypt(objectId, item) {
        logger_1.Logger.log.trace(`ClocoClient.encodeAndEncrypt: processing object '${objectId}'.`);
        // retrieve the config object metadata.
        let configObject = this.getConfigObject(objectId);
        if (!configObject) {
            logger_1.Logger.log.trace(`ClocoClient.encodeAndEncrypt: config object metadata for '${objectId}' not found in application.`, this.app);
        ...
        if (configObject.encrypted) {
            ...
            logger_1.Logger.log.trace(`ClocoClient.encodeAndEncrypt: encoding object '${objectId}' in format '${configObject.format}'.`);
            let encoded = encoder.encode(item);
            logger_1.Logger.log.trace(`ClocoClient.encodeAndEncrypt: encrypting object '${objectId}'.`);
            data = this.options.encryptor.encrypt(encoded);
            logger_1.Logger.log.trace(`ClocoClient.encodeAndEncrypt: object '${objectId}' encoded and encrypted.`, { data: data });
        }
        else {
            // if not encrypted, pass straight through.
            logger_1.Logger.log.trace(`ClocoClient.encodeAndEncrypt: object '${objectId}' not encrypted, passing through.`, { data: item });
            data = item;
        }
        return data;
    }
    ...
    decryptAndDecode(wrapper) {
        logger_1.Logger.log.trace(`ClocoClient.decryptAndDecode: processing object '${wrapper.objectIdentifier}'.`);
        // retrieve the config object metadata.
        let configObject = this.getConfigObject(wrapper.objectIdentifier);
        if (!configObject) {
            logger_1.Logger.log.trace(`ClocoClient.decryptAndDecode: config object metadata for '${wrapper.objectIdentifier}' not found in application.`, this.app);
            throw new Error(`Config object metadata for '${wrapper.objectIdentifier}' not found in application.`);
        }
        // check if the data is encrypted.
        let data;
        if (configObject.encrypted) {
            logger_1.Logger.log.trace(`ClocoClient.decryptAndDecode: decrypting object '${wrapper.objectIdentifier}'.`);
            let decrypted = this.options.encryptor.decrypt(wrapper.configurationData);
            logger_1.Logger.log.trace(`ClocoClient.decryptAndDecode: decoding object '${wrapper.objectIdentifier}' to format '${configObject.format}'.`);
            let encoder = encoding_1.Encoding.getEncoder(configObject.format);
            data = encoder.decode(decrypted);
            logger_1.Logger.log.trace(`ClocoClient.decryptAndDecode: object '${wrapper.objectIdentifier}' decrypted and decoded.`, { data: data });
        }
        else {
            // if not encrypted, pass straight through.
            logger_1.Logger.log.trace(`ClocoClient.decryptAndDecode: object '${wrapper.objectIdentifier}' not encrypted, passing through.`, { data: wrapper.configurationData });
